...
run_mcmc_sampler<-function(M,myseed,config_number,plot_iter=1000,load_data=load_spelling_data,longitudinal=T)
{
...
run_mcmc_sampler(M,myseed,config_number,plot_iter=plot_iter,load_data=load_data,longitudinal=T)

save_filename=file.path(paste('Saved_output/saved_output_config_',config_number,'_seed_',111,'_data_',dataname,sep=''))
load(save_filename,verb=T)
print(sapply(acceptance_rates,function(x) mean(x,na.rm=T)))
plot_fun(stored_parameters,223,mytitle=toString(jj),save_fig=T,save_filename=paste(plot_dirname,'/iteration_',jj,'.png',sep=''))
plot_fun(stored_parameters,224,mytitle=toString(jj),save_fig=T,save_filename=paste(plot_dirname,'/iteration_',jj,'.png',sep=''))

for(name in names(stored_parameters)){current_values[[name]]=stored_parameters[[name]][[min(length(stored_parameters[[name]]),220)]]}
mumu=current_values$mu
