...
plot_latent_longitudinal <- function(stored_parameters,store_index,mytitle="",save_fig=F,save_filename="",cluster=T)
{
  if(mytitle=="")
  {
    mytitle=paste('Latent space sample at k=',store_index,sep='')
  }
  z1=data.frame(stored_parameters$z1[[store_index]])
  names(z1)=c('coord1','coord2')
  z2=data.frame(stored_parameters$z2[[store_index]])
  names(z2)=c('coord1','coord2')
  w=data.frame(stored_parameters$w[[store_index]])
  names(w)=c('coord1','coord2')
  w$name=sapply(1:nw,toString)
  ...
  p0<-ggplot()+geom_point(aes(x=coord1,y=coord2,col=time),z,cex=2)
  p0<-p0+xlab('coordinate 1')+ylab('coordinate 2')+ggtitle(mytitle)
  p0<-p0+geom_text(aes(x=coord1,y=coord2,label=name),hjust=2,vjust=2,data=w,fontface="bold")
  p0<-p0+geom_path(aes(x=coord1,y=coord2,group=grouping),data=z,cex=.2,linetype='dotted')
  if(cluster)
  {
    p0<-p0+geom_point(aes(x=coord1,y=coord2),pch=8,col='black',data=mu)
    p0<-p0+geom_circle(aes(x0=coord1,y0=coord2,r=r),data=mu)  
  ...
  # {
    print(p0)
  # }
  # p1=p0+geom_point(aes(x=current_values[['mu_z']][,1],y=current_values[['mu_z']][,2],col=c("1_z","2_z","3_z","4_z","5_z")),cex=4)
}
...
update_K <- function(varname="")
{
  ...
  probs_z1=apply(current_values$z1,1,function(xx) dnorm(xx[1],mus[,1],sigmas)*dnorm(xx[2],mus[,2],sigmas)*lambdas)
  probs_z2=apply(current_values$z2,1,function(xx) dnorm(xx[1],mus[,1],sigmas)*dnorm(xx[2],mus[,2],sigmas)*lambdas)
  probs_z=probs_z1*probs_z2
...
update_stored_vars<-function()
{
  ...
  {
    temp=rbind(current_values$z1[current_values$K_z==ii,],current_values$z2[current_values$K_z==ii,])
    gms[ii]=dim(temp)[1]
    if(gms[ii]<=1)
    {
      gmeans[ii,]=c(0,0)
