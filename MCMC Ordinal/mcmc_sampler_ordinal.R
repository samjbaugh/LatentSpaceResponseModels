...
require('pdist')
require('ggforce')
source('update_functions_ordinal.R')
source('likelihood_functions.R')
source('init_ordinal.R')

run_mcmc_sampler<-function(M,myseed,config_number,plot_iter=100,load_data=load_charity_data)
{
  set.seed(myseed)
  ...
  save_filename=file.path(paste('Saved_output/saved_output_config_',config_number,'_seed_',myseed,'_data_',dataname,'.Rdat',sep=''))
  ...
  runtimes=system.time({
    for(jj in start_index:M)
    {
      ...
      if(jj%%plot_iter==0)
      {
        plot_latent_ordinal_cluster(stored_parameters,jj,save_tf=T,save_filename=paste(plot_dirname,'/iteration_',jj,'.png',sep=''))
